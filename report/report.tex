...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily 共源 NMOS 放大器 SPICE 仿真器}
\fancyhead[R]{\small\sffamily 器件模型与 SPICE 仿真}
...
\begin{titlepage}
  \centering
  \vspace*{3cm}
  {\LARGE\bfseries\sffamily 共源 NMOS 放大器\\[0.4em]
   SPICE 仿真器设计报告\par}
  \vspace{1.2cm}
  \rule{0.6\linewidth}{0.4pt}
  \vspace{1.2cm}

  {\large 器件模型与 SPICE 仿真\par}
  \vspace{0.4em}
  {\normalsize 复旦大学 微电子学院 \quad 2026 春\par}

  \vspace{2.5cm}
  {\normalsize
  \begin{tabular}{rl}
    课程代码 & MICS3006 \\[0.3em]
    代码仓库 & \url{https://github.com/LArielOzjH/nano-spice} \\[0.3em]
    仿真语言 & Python 3 (NumPy + Matplotlib) \\
  \end{tabular}
  \par}

  \vfill
  {\small 本报告由仿真代码自动生成的图形及手工推导的公式共同构成。}
\end{titlepage}
...
\begin{itemize}
  ...
  \item 所有求解收敛判据与商业 SPICE（如 PSpice）保持一致。
\end{itemize}
...
负载电阻 $R_D = 10\,\mathrm{k\Omega}$ 接于 $V_{DD}=5\,\mathrm{V}$ 与输出节点 VOUT 之间；
...
\begin{circuitikz}[scale=1.1, american]
  % VDD node
  \node[above] at (3, 4.5) {$V_{DD} = 5\,\mathrm{V}$};
  \draw (3, 4.5) -- (3, 4.0);
  % RD
  \draw (3, 4.0) to[R, l=$R_D{=}10\,\mathrm{k}\Omega$, v^<=$V_{R_D}$] (3, 2.2);
  % VOUT node
  \node[right] at (3.15, 2.2) {VOUT ($V_2$)};
  \filldraw (3, 2.2) circle (1.5pt);
  % NMOS
  \draw (3, 2.2) -- (3, 1.9);
  \draw (3, 1.9) node[nmos, anchor=drain] (mos) {};
  % Gate
  \draw (mos.gate) -- ++(-1.0, 0) node[left] {$V_{IN}$ ($V_1$)};
  % Source to GND
  \draw (mos.source) -- (3, 0) node[ground] {};
  % VIN source
  \draw (0, 0) node[ground] {} to[V, l=$V_{IN}$, invert] (0, 1.9) -- (mos.gate |- 0, 1.9) -- (mos.gate);
  % VDD source
  \draw (5, 0) node[ground] {} to[V, l=$V_{DD}$, invert] (5, 4.5) -- (3, 4.5);
  % Labels
  \node[right] at (3.6, 1.3) {M0};
  \node[right, font=\small] at (3.6, 0.9) {$V_{th}{=}0.8\,\mathrm{V}$};
  \node[right, font=\small] at (3.6, 0.55) {$k_p{=}100\,\mu\mathrm{A/V^2}$, $W/L{=}10$};
\end{circuitikz}
\caption{共源 NMOS 放大器电路图。当考虑源极/漏极寄生电阻时，
在节点 VOUT 与 M0 漏极之间插入 $R_d$（本征漏极节点 $V_5$），
在 M0 源极与地之间插入 $R_s$（本征源极节点 $V_4$）。}
\label{fig:circuit}
\end{figure}

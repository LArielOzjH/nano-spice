...
\begin{figure}[H]
\centering
\begin{circuitikz}[american, scale=1.15, font=\small]

  % ── MOSFET ──────────────────────────────────────────
  \node[nmos] (M) at (3, 2.5) {};
  \node[right=4pt] at (M.base) {M0};

  % ── Drain → RD → VDD ────────────────────────────────
  \draw (M.drain) -- ++(0, 0.5) coordinate (dout)
        to[R, l={$R_D{=}10\,\mathrm{k\Omega}$}] ++(0, 2.2)
        node[above] {$V_{DD}{=}5\,\mathrm{V}$};

  % ── VOUT tap ────────────────────────────────────────
  \filldraw (dout) circle (1.6pt);
  \draw (dout) -- ++(1.6, 0) node[right] {VOUT \ ($V_2$)};

  % ── Source → GND ────────────────────────────────────
  \draw (M.source) -- ++(0, -0.5) node[ground] {};

  % ── VIN source → Gate ───────────────────────────────
  \draw (1, 0.7) node[ground] {}
        to[V, l=$V_{IN}$] (1, 2.5) -- (M.gate);
  \node[above=1pt, left=2pt] at (M.gate) {$V_1$};
